...
def _build_chat_prompts(
    *,
    ...
    system = (
        ...
        "Do NOT mention sources, RAG, 'context' or citations. "
        "No markdown, no lists, no extra commentary. "
        "Use LIVE WEATHER for the actual conditions. "
        "Use RAG CONTEXT as the source of upcoming events. "
        "Mention 1–2 specific upcoming events found in RAG CONTEXT (include date/time/place if present). "
        "Do NOT invent event details not present in RAG CONTEXT. "
        "If an event in RAG CONTEXT includes an official URL, you MAY include at most one URL. "
        "Translate Japanese context to English. "
        f"If you add hashtags, pick 1-3 from: {hashtags}. "
        "Be warm, upbeat, and practical."
    )
